...
/*   Updated: 2022/05/10 11:40:19 by nposidon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || fd > 1023)
		return (0);
	if (start[fd] == 0 || fd_len((t_list2 *) start[fd]) == -1)
		error = read_text(fd, (t_list2 **) &(start[fd]));
	if (error == -1)
	{
		ft_free((t_list2 **) &(start[fd]), 0);
		return (0);
	}
	len = fd_len((t_list2 *) start[fd]);
	if (len == 0)
		return (ft_free((t_list2 **) &(start[fd]), 0));
	out = malloc(len + 1);
	if (out && !ft_coppy((t_list2 **) &(start[fd]), out, &len))
		ft_free((t_list2 **) &(start[fd]), &out);
	return (out);
}
